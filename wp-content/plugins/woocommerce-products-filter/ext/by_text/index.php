...
final class WOOF_EXT_BY_TEXT extends WOOF_EXT {
    ...
    public function posts_join($join, $wp_query) {
        ...
        if ($meta_search) {
            $join .= " LEFT JOIN $wpdb->postmeta AS postmeta ON ( wp_posts.ID = postmeta.post_id )";
        }
        ...
        foreach ($search_terms as $term) {
            ...
            if ($meta_search && !empty($meta_search_terms)) {
                foreach ($meta_search_terms as $meta_term) {
                    $sub_sql[] = $wpdb->prepare("( postmeta.meta_key = '%s' AND  postmeta.meta_value $search_type %s)", $meta_term, $word);
                }
            }
